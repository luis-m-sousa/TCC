...
function gerarGraficoBarra() {
    ...
    for (let i = 0; i < 12; i++) {
        let n = (i + 1) * 6;
        parcelas[i] = ((valor * taxa / 100) / (1 - Math.pow(1 + taxa / 100, (-n)))).toFixed(2);
    }

    for (let i = 0; i < parcelas.length; i++) {
        let n = (i + 1) * 6;
        totalJuros[i] = (parcelas[i] * n - valor).toFixed(2);
    }
    ...
    // Crie o novo gráfico
     window.barraChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: Array.from({length: 12}, (_, i) => (i + 1) * 6),
            datasets: [{
                label: 'Parcelas',
                data: parcelas,
